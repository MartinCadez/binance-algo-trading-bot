...
pub async fn clear_trades_table(pool: &PgPool) -> Result<u64, sqlx::Error> {
    let result = sqlx::query("DELETE FROM trades")
        .execute(pool)
        .await?;
...
pub async fn add_price(pool: &PgPool, candle_stick: CandleStick, m: i32) -> Result<CandleStick, sqlx::Error> {
    // Insert new price
...
pub async fn get_last_n_prices(pool: &PgPool, n: i64) -> Result<Vec<CandleStick>, sqlx::Error> {
    let result = sqlx::query_as::<_, CandleStick>(
...
pub async fn delete_price(pool: &PgPool) -> Result<u64, sqlx::Error> {
    let result = sqlx::query("DELETE FROM prices")
        .execute(pool)
        .await?;
