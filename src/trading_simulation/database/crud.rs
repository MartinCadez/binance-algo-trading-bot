...
//     let db_pool = connect_to_database(&database_url).await.expect("Connection failed");
...
//     let last_trade = crud::get_last_trade(&db_pool).await.expect("Failed to load last trade");
//     println!("Last Trade: {:?}", last_trade);

//     let last_3_prices = crud::get_last_n_prices(&db_pool, 3).await.expect("Failed to load prices");
